...
- `p3971` — Jetson AGX Thor Devkit 载板（兼容 `p3834` 板子）
...
- 虚拟通道（VC）：通过 `vc-id` 区分，每路相机对应独立 VC（4CH 板子使用 VC 0/1/2/3）
- I2C 总线：
  - `i2c@810c6a0000`（Bus-9）：接入 `0x3a` / `0x3b`（2CH、4CH 板子前两组）
  - `i2c@810c6d0000`（Bus-12）：接入 `0x3a` / `0x3b`（2CH、4CH 板子后两组）；4CH 还会扩到 `0x3c` / `0x3d`
- CSI：`port-index = <2>` 对应 `serial_c`、`port-index = <0>` 对应 `serial_a`
...
- 2CH 板子 → 使用 `*-fg-2ch-overlay.dts`
- 4CH 板子 → 使用 `*-fg-4ch-overlay.dts`
